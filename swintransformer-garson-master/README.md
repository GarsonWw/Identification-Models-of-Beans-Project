## 代码使用简介
💡 根文件夹为swin_transformer
1. 如果要使用自己的数据集，请按照`beans_data`文件结构进行摆放(即一个类别对应一个文件夹)，并且将训练以及预测脚本中的`num_classes`设置成你自己数据的类别数
2. 在`train.py`脚本中将`--data-path`设置成解压后的`beans_photos`文件夹绝对路径
