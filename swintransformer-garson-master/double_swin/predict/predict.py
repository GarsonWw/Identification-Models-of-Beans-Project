...
def main(myfile):
    ...
    # load model weights
    model1_weight_path = "../train/weights/best_epoch1.pth"
    net1.load_state_dict(torch.load(model1_weight_path, map_location=device))
    
    # create net2
    net2 = create_model(num_classes=2).to(device)
    # load model weights
    model2_weight_path = "../train/weights/best_epoch2.pth"
    net2.load_state_dict(torch.load(model2_weight_path, map_location=device))
...
if __name__ == '__main__':
    myfile = open(
        'G:/swintransformer-garson-master/double_swin/predict/dswin_pred_result2.csv',
        'w')
    main(myfile)
